...
namespace packet_analyzer
{
	...
	class pause_packet_handler : public abstract_handler
	{
	public:
		pause_packet_handler(std::shared_ptr<std::shared_ptr<i_state>> state,
		                     std::shared_ptr<spsc_packet_queue> packet_queue);

		void handle(const std::shared_ptr<i_packet> packet) override;

	private:
		static bool is_pause_packet(std::shared_ptr<i_packet> const& packet);
